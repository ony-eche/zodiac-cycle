...
import {
  ...
  eachDayOfInterval, isSameDay, addMonths, subMonths, isToday, startOfWeek, endOfWeek
} from 'date-fns';
import { X, Check, Trash2, Edit3, Droplet, ChevronLeft, ChevronRight } from 'lucide-react';
...
function getDayStatus(date: Date, periods: PeriodEntry[], cycleInfo: any): 'period' | 'predicted' | 'fertile' | 'ovulation' | 'normal' {
...
const PHASE_INSIGHTS: Record<string, { emoji: string; label: string; color: string; tip: string; energy: string; mood: string }> = {
  menstrual: {
    emoji: '🔴', label: 'Menstrual', color: '#f43f5e',
    tip: 'Rest is productive. Your body is working hard — honour it with warmth, gentle movement, and iron-rich foods.',
    energy: 'Low', mood: 'Reflective',
  },
  follicular: {
    emoji: '🌸', label: 'Follicular', color: '#c084fc',
    tip: 'Estrogen is rising! Great time to start new projects, socialise, and try new workouts.',
    energy: 'Rising', mood: 'Optimistic',
  },
  ovulation: {
    emoji: '⭐', label: 'Ovulation', color: '#f59e0b',
    tip: 'Peak fertility and confidence. You\'re magnetic right now — ideal for big presentations and important conversations.',
    energy: 'Peak', mood: 'Social',
  },
  luteal: {
    emoji: '🌙', label: 'Luteal', color: '#818cf8',
    tip: 'Progesterone peaks then drops. Focus on completing tasks, eating magnesium-rich foods, and winding down.',
    energy: 'Declining', mood: 'Introspective',
  },
  unknown: {
    emoji: '✨', label: 'Unknown', color: '#c084fc',
    tip: 'Log your period dates to unlock personalised cycle insights and predictions.',
    energy: '—', mood: '—',
  },
};
...
function MonthGrid({ month, periods, logs, cycleInfo, editMode, selectedDate, onDayTap, onEditTap }: {
  ...
  return (
    <div className="mb-8">
      {/* Month name */}
      <p className="text-center text-base font-bold text-gray-800 mb-4">{format(month, 'MMMM yyyy')}</p>
      ...
      <div className="grid grid-cols-7 gap-y-1">
        ...
        {days.map(day => {
          ...
          // Find if this day belongs to a period entry (for edit button)
          const periodEntry = periods.find(p => {
            const s = new Date(p.start);
            const e = p.end ? new Date(p.end) : addDays(s, 4);
            return day >= s && day <= e;
          });
          ...
          return (
            <div key={dateStr} className="flex flex-col items-center py-0.5">
              {/* TODAY label */}
              {isToday_ && (
                <span className="text-[8px] font-bold text-gray-500 uppercase tracking-widest mb-0.5">Today</span>
              )}

              {/* Day number above circle */}
              {!isToday_ && (
                <span className={`text-xs mb-0.5 font-medium ${isFuture ? 'text-gray-300' : 'text-gray-700'} ${isFertile ? 'text-teal-500' : ''} ${isOvulation ? 'text-teal-600' : ''} ${isPredicted ? 'text-rose-300' : ''}`}>
                  {format(day, 'd')}
                </span>
              )}
              {isToday_ && (
                <span className="text-xs mb-0.5 font-bold text-gray-800">{format(day, 'd')}</span>
              )}
              ...
              <button
                ...
                className={`
                  ...
                  ${isPeriod ? 'bg-rose-400 shadow-md shadow-rose-200' : ''}
                  ${isPredicted && !isPeriod ? 'border-2 border-dashed border-rose-300' : ''}
                  ${isFertile && !isPeriod ? 'border-2 border-teal-300' : ''}
                  ${isOvulation && !isPeriod ? 'border-2 border-teal-400 bg-teal-50' : ''}
                  ${!isPeriod && !isPredicted && !isFertile && !isOvulation ? 'border-2 border-gray-200 hover:border-rose-300' : ''}
                  ${isSelected && !isPeriod ? 'ring-2 ring-rose-400 ring-offset-2' : ''}
                  ${isToday_ && !isPeriod ? 'border-gray-700 border-2' : ''}
                  ${editMode && isPeriod ? 'scale-95' : ''}
                `}
              >
                {isPeriod && (
              ...
              {/* Edit button for period days */}
              {isPeriod && periodEntry && !editMode && (
                <button
                  onClick={e => { e.stopPropagation(); onEditTap(periodEntry); }}
...
function SymptomTrends({ logs }: { logs: Record<string, DayLog> }) {
  ...
  const max = top[0][1];
  return (
    <div className="rounded-3xl p-5 border border-rose-100 bg-white">
...
export function CycleTab() {
  ...
  // Generate months: 2 years back, current, 3 months forward
  ...
  return (
    <div className="pb-28" style={{ background: 'transparent' }}>
      ...
      {/* ── Phase summary card ── */}
      <div className="mx-0 mb-5 rounded-3xl p-5 border border-rose-100 bg-white shadow-sm shadow-rose-100">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="relative flex-shrink-0">
            <svg width="80" height="80" viewBox="0 0 80 80">
              ...
              />
              <text x="40" y="36" textAnchor="middle" fontSize="18" fontWeight="bold" fill="#1f2937">{cycleInfo.cycleDay}</text>
        ...
        <div className="flex gap-2 mt-3">
          <div className="flex-1 rounded-xl p-2.5 text-center bg-gray-50">
            <p className="text-[10px] text-gray-400 uppercase tracking-wide">Energy</p>
            <p className="text-xs font-bold text-gray-700 mt-0.5">{phase.energy}</p>
          </div>
          <div className="flex-1 rounded-xl p-2.5 text-center bg-gray-50">
            <p className="text-[10px] text-gray-400 uppercase tracking-wide">Mood</p>
            <p className="text-xs font-bold text-gray-700 mt-0.5">{phase.mood}</p>
          </div>
          <div className="flex-1 rounded-xl p-2.5 text-center bg-gray-50">
            <p className="text-[10px] text-gray-400 uppercase tracking-wide">Cycles logged</p>
            <p className="text-xs font-bold text-gray-700 mt-0.5">{periods.length}</p>
          </div>
        </div>
      </div>
      ...
      {/* ── Edit mode banner ── */}
      {editMode && (
        <div className="mx-0 mb-3 rounded-2xl p-3 border border-rose-300 text-center bg-rose-50">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1.5 mb-4 px-1">
        {[
          ...
          { color: 'bg-white border-2 border-dashed border-rose-300', label: 'Predicted' },
          { color: 'bg-white border-2 border-teal-300', label: 'Fertile' },
          { color: 'bg-teal-50 border-2 border-teal-400', label: 'Ovulation' },
        ].map(l => (
      ...
      {/* ── Scrollable calendar ── */}
      <div ref={scrollRef} className="rounded-3xl bg-white border border-rose-100 shadow-sm shadow-rose-50 px-4 pt-4 pb-2 mb-4">
        {/* Header controls */}
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="text-xs px-3 py-1.5 rounded-full border border-rose-200 text-rose-400 font-medium hover:bg-rose-50 transition-colors"
            >
              Today
            </button>
            <button
        ...
        {/* Month grids */}
        <div className="max-h-[500px] overflow-y-auto pr-1" style={{ scrollbarWidth: 'none' }}>
      ...
      {/* ── Period history ── */}
      <div className="rounded-3xl bg-white border border-rose-100 shadow-sm p-5 mb-4">
        <h3 className="text-sm font-bold text-gray-700 mb-3">Period History</h3>
        {periods.length === 0 ? (
          <div className="text-center py-6">
            <p className="text-3xl mb-2">🩸</p>
            <p className="text-sm text-gray-400">No periods logged yet</p>
            <p className="text-xs text-gray-300 mt-1">Tap "Edit Period" and tap days to mark your period</p>
          </div>
        ) : (
          <div className="space-y-2">
            {[...periods]
              ...
              .slice(0, 8)
              .map(p => (
                <div key={p.id} className="flex items-center justify-between py-2.5 px-3 rounded-2xl bg-rose-50 border border-rose-100">
                  <div className="flex items-center gap-3">
                    <div className="w-2.5 h-2.5 rounded-full bg-rose-400 shadow-sm shadow-rose-200" />
                    <div>
                      <p className="text-sm font-semibold text-gray-700">
                        {format(new Date(p.start), 'MMM d')} – {p.end ? format(new Date(p.end), 'MMM d') : '?'}
                      </p>
                      <p className="text-[10px] text-gray-400">{format(new Date(p.start), 'yyyy')}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {p.end && (
                      <span className="text-xs text-rose-400 font-semibold bg-white px-2 py-0.5 rounded-full border border-rose-100">
                        {differenceInDays(new Date(p.end), new Date(p.start)) + 1}d
                      </span>
                    )}
                    <button onClick={() => setEditingPeriod(p)} className="p-1.5 hover:bg-rose-100 rounded-lg transition-colors">
